...
export function Progress() {
  ...
  const modeLabels: Record<string, string> = {
    "review-real-work": "Review Real Work",
    "inbox-fire": "🔥 Inbox Fire",
    "red-pen": "✂️ Red Pen",
    ...
    return (
      <div className="space-y-8 text-center pt-16">
        ...
        <p className="text-dojo-muted">
          Complete your first round to start tracking progress.
        </p>
      ...
      <div className="dojo-card">
        ...
        <div className="space-y-2">
          {recent.map((s, i) => (
            <div
              key={i}
              className="flex items-center justify-between text-sm py-2 border-b border-dojo-border last:border-0"
            >
              <div>
              ...
              <div className="text-dojo-accent font-bold">
                {s.scores.total ??
                  Object.values(s.scores).reduce((a, b) => a + (b ?? 0), 0)}
              </div>
            </div>
          ))}
        </div>
